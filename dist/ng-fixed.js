      ...
      link: function link($scope, element) {
        ...
        var updateElement = function updateElement() {
          var pageYOffset = window.pageYOffset || document.scrollTop,
              ...
              elementBottomYOffset;
          var yOffset = pageYOffset - clientTop;

          fixedElement = document.querySelector($scope.fixedElementSelector);

          if (fixedElement) {
            fixedElementYOffset = fixedElement.getBoundingClientRect().top + pageYOffset;
          }

          if ($scope.ngFixed !== false && fixedElementYOffset && yOffset >= fixedElementYOffset) {

            if ($scope.unfixedElementSelector) {
              unfixedElement = document.querySelector($scope.unfixedElementSelector);
              if (unfixedElement) {
                unfixedElementYOffset = unfixedElement.getBoundingClientRect().top + pageYOffset;
                elementBottomYOffset = elementRect.height + pageYOffset;
              }
            }

            if (unfixedElementYOffset && unfixedElementYOffset <= elementBottomYOffset) {
              element.css('top', unfixedElementYOffset - elementRect.height + 'px');
              element.removeClass($scope.fixedClass);
              added = false;
            } else if (!added && (!unfixedElementYOffset || unfixedElementYOffset >= elementBottomYOffset)) {
              element.css('top', '');
